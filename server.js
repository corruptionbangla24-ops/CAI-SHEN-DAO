...
// 🧧 ওরিজিনাল চাই শেন দাও ৫টি লাক্সারি ক্যাসিনো সিম্বল ম্যাপ
const caishenSymbolsPool = ["RED_POCKET", "GOLD_BAG", "CAISHEN_GOD", "GOLDEN_INGOT", "LANTERN_RED"];
...
app.get('/api/slot-balance', async (req, res) => {
    ...
    try {
        const response = await axios.post(`${MAIN_SITE_URL}/api_callback.php`, {
            action: "balance", username: finalUser, amount: 0, wallet: targetWallet, game: "caishendao"
        }, { timeout: 15000 });
...
// 🛫 ২. চাই শেন দাও কোর ৫-রিল স্পিন রাউট (মানি ট্রি ও ফ্যান-টানের মতো ১০০% সিকিউরড সিঙ্গেল পাইপライン প্রোটোকল)
app.post('/api/slot-spin', async (req, res) => {
    const { userId, amount, wallet } = req.body; 
    const reqAmount = parseFloat(amount) || 50; // ডিফল্ট ৫০ টাকা স্টেক লক চ্যাম
    const finalGameName = "caishendao"; 
    const targetWallet = wallet || "main";
    ...
    try {
        ...
        while (isLoopActive && loopSafety < 150) {
            ...
            for (let i = 0; i < 5; i++) {
                let randomIdx = Math.floor(Math.random() * caishenSymbolsPool.length);
                finalReelsResultMatrix.push(caishenSymbolsPool[randomIdx]);
            }
            ...
            if (maxMatchesCount === 5) {
                ...
                if (matchedSymbolName === "RED_POCKET") winMultiplier = 50.00;       
                else if (matchedSymbolName === "GOLD_BAG") winMultiplier = 25.00; 
                else if (matchedSymbolName === "CAISHEN_GOD") winMultiplier = 20.00; 
                else if (matchedSymbolName === "GOLDEN_INGOT") winMultiplier = 15.00; 
                else winMultiplier = 12.00;                          
                finalStatus = "win";
            } else if (maxMatchesCount === 4) {
                winMultiplier = (matchedSymbolName === "RED_POCKET") ? 8.00 : 4.00;
                finalStatus = "win";
            } else if (maxMatchesCount === 3) {
                winMultiplier = (matchedSymbolName === "RED_POCKET") ? 2.50 : 1.50;
                finalStatus = "win";
            ...
            if (balResponse.data && balResponse.data.caishendao_target) {
                let target = String(balResponse.data.caishendao_target).toUpperCase();
                if (target === "FORCE_LOSE" && finalStatus === "win") {
                    finalReelsResultMatrix = ["RED_POCKET", "GOLD_BAG", "CAISHEN_GOD", "GOLDEN_INGOT", "LANTERN_RED"];
...
server.listen(PORT, () => { console.log(`🧧 Cai Shen Dao Royal 5-Reel Slots Engine Running on port ${PORT}`); });
