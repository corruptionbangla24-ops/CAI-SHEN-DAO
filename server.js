...
// 🧧 Five-রিলস ওরিজিনাল কাই শেন দাও লাক্সারি ক্যাসিনো র্যান্ডমাইজেশন প্রতীক তালিকা ভাই ভাই
const caiShenDaoPool = ["CAISHEN_GOD", "GOLDEN_BAG", "FORTUNE_KNOT", "RED_LANTERN", "GOLDEN_COIN", "JADE_POT", "LUCKY_SCROLL"];

// 💰 ১. লাইভ অ্যাকাউন্ট ব্যালেন্স নিয়ে আসার ডেডিকেটেড গেটওয়ে
app.get('/api/caishen-balance', async (req, res) => {
...
// 🛫 ২. কাই শেন দাও ৫-রিল কোর স্লট স্পিন রাউট (POST Route - ৯৫% RTP গাণিতিক বর্ম কঠোর লক ভাই ভাই!)
app.post('/api/caishen-spin', async (req, res) => {
    ...
    try {
        ...
        let adminTriggeredPrize = (balResponse.data && balResponse.data.caishen_target) ? balResponse.data.caishen_target : null;

        let spinResults, finalStatus, winMultiplier;
        let isLoopActive = true;
        let loopSafety = 0;

        // 🎰 [🎰 ৯৫% ওরিজিনাল ক্যাসিনো RTP ও ৫-রিলস ফরচুন গড গাণিতিক লুপ ভাই ভাই]
        while (isLoopActive && loopSafety < 200) {
            ...
            spinResults = [];
            for (let r = 0; r < 5; r++) {
                spinResults.push(caiShenDaoPool[Math.floor(Math.random() * caiShenDaoPool.length)]);
            }
            ...
            if (maxSameSymbolsCount >= 3) {
                finalStatus = "win";
                // 🚀 [ডাইনামিক কম্বো মাল্টিপ্লায়ার ওডস]: ৩টি মিললে ৬ গুণ, ৪টি মিললে ৪০ গুণ, ৫টি মিললে ৪০০ গুণের গ্র্যান্ড ফরচুন জ্যাকпот!
                if (maxSameSymbolsCount === 3) winMultiplier = 6.00;
            ...
            if (adminTriggeredPrize) {
                ...
                // ৪০০ গুণের মেগা জ্যাকপটের চান্স আরটিপি লুপ ট্র্যাকে কড়া সুরক্ষায় টাইট ০.১% এ লক ভাই ভাই
        ...
        console.error("Cai Shen Dao Core Engine Error:", e.message);
...
io.on('connection', (socket) => { console.log("Player connected to Royal Cai Shen Dao 5-Reel Slot Engine!"); });

// কাই শেন দাও গেম নিজস্ব কাস্টম ৬২০০ পোর্টে কড়া নিয়নে অন ফায়ার ভাই ভাই!
const PORT = process.env.PORT || 6200; 
server.listen(PORT, () => { console.log(`🎡 Royal Cai Shen Dao Engine Running on port ${PORT}`); });
